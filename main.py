...
Cleans raw text so it matches then format used while training.
A.Convert the text to lowercase.
B.Remove apostrophes(eg.can't)
C.Remove special Characters and Punctutation.
...
class TextInput(BaseModel):
    text : str = Field(
        ...
        json_schema_extra={'example': 'I feel so happy and exited'}
    )
...
#Model will keep running until we close server
Model does to work : 1.Run before server starts, 2.Run after server closes.
@asynccontentmanager
